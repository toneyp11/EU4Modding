...
| Loc string renders blank; `error.log` full of `Unknown text property 'GetValue]'` | **two or more `[x.y.GetValue]` in ONE loc string.** The parser mis-terminates the first bracket when another follows. Measured: 9,261 errors from three status lines in one run; vanilla uses this form exactly **once** in the entire game | **one scope command per loc string** - split each value into its own `custom_text_box`. `check-mod.sh` now fails on 2+ per string |
...
| Two nations trade the same province back and forth every run | a transfer that flips which of the pair is the extreme target. Taking from a neighbour only slightly bigger makes the DONOR the new weakest, so the next run reverses it | give the beneficiary a time-limited cooldown flag (`set_country_flag` + `had_country_flag = { flag = X days = 3650 }`) and exclude recently-boosted nations from being donors. Breaks the cycle without skipping either nation |
